...
[Script]
cron "0 0/30 * * *" script-path=kbg_txzxg.js, tag=腾讯自选股V2零点提现, enabled=true
*/
const $ = new Env('腾讯自选股V2零点提现');
const jsname = '腾讯自选股V2零点提现';
...
class UserInfo {
    ...
    async withdraw(ticket,item_id) {
        try {
            ...
            console.log(result)
            if(result.retcode==0) {
                logAndNotify(`提现结果：${result.retmsg === 'OK'?'提现成功,请查看微信收款通知':result.retmsg}`);
            } else {
