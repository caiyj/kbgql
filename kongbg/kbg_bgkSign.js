...
ck就是 appwk.baidu.com 域名下任意请求里的整个完整cookie
自己抓包不挂科
===========================
[Script]
...
!(async () => {
    ...
    if (!cookieArr[0]) {
        if($.isNode()){
            ...
            console.log('运行通知\n\n没有获取到配置的cookie信息')
            $.msg($.name, '运行通知\n\n没有获取到配置的cookie信息', 'https://bean.m.jd.com/bean/signIndex.action', { "open-url": "https://bean.m.jd.com/bean/signIndex.action" });
        }
        return
    }
