/*
初始化${repoName}仓库
初始化${repoName}仓库一些必要文件
更新时间：2022-3-27
脚本兼容: Node.js
================Loon==============
[Script]
cron "0,30 * * * *" script-path=kongbg_kbgql_init.js, tag=初始化${repoName}仓库
*/
const $ = new Env("初始化kongbg_kbgql");
const fs = require('fs')
...
!(async () => {
    console.log(`开始初始化${repoName}\n`)
    console.log(`开始检查${configPath}/scripts/pushToken.json 是否存在\n`)
    const pushTokenExist = await isFileExisted(`${configPath}/scripts/pushToken.json`);
    if (!pushTokenExist) {
        console.log(`${configPath}/scripts/pushToken.json不存在\n`)
        console.log(`开始从${configPath}/repo/${repoName}中复制过来\n`)
        const str = `cp ${configPath}/repo/${repoName}/pushToken.json ${configPath}/scripts/`
        const {error, stdout} = await execFn(str);
        if (!error) {
            console.log("复制成功\n");
        } else {
            console.log("复制失败，请手动处理\n");
        }
    } else {
        console.log('已存在，跳过处理\n')
    }

    console.log(`青龙自带通知脚本拉库后会覆盖自己的通知脚本，${configPath}/deps/下的sendNotify.js 老是会覆盖当前目录下的sendNotify.js\n`)
    console.log(`懒得去研究了，直接用当前项目的sendNotify.js覆盖掉${configPath}/deps/下的sendNotify.js\n`)
    console.log(`当前项目的sendNotify.js支持pushplus一对一精准信息推送\n`)
    
    const str1 = `cp ${configPath}/repo/${repoName}/sendNotify.js ${configPath}/deps/`
    const {error1, stdout1} = await execFn(str1);
    if (!error1) {
        console.log(`复制到${configPath}/deps/成功\n`);
    } else {
        console.log("复制失败，请手动处理\n");
    }
    ...
    if (!error2) {
        console.log(`复制到${configPath}/scripts/成功\n`);
    } else {
        console.log(`复制失败，请手动处理\n`);
    }
    ...
    if (!error3) {
        console.log(`复制到${configPath}/scripts/${repoName}/成功\n`);
    } else {
        console.log("复制失败，请手动处理\n");
    }

    console.log(`当前项目的notify.py覆盖掉${configPath}/deps/下的notify.py\n`)
    console.log(`当前项目的notify.py支持pushplus一对一精准信息推送\n`)
    ...
    console.log(`初始化${repoName}完成.\n`);
    console.log(`若是失败，请手动处理\n`);
})()
