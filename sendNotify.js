 ...
 * @Last Modified by: kongbg
 * @Last Modified time: 2022-3-27 19:00:00
 * sendNotify 推送通知功能
 * @param text 通知头
 * @param desp 通知体
...
function pushPlusNotify1(text, desp, PUSH_PLUS_TOKEN=PUSH_PLUS_TOKEN) {
  return new Promise((resolve) => {
    if (PUSH_PLUS_TOKEN) {
      ...
      console.log('PUSH_PLUS_TOKEN为空，信息推送失败！')
      resolve();
    }
  });
}
...
async function pushPlusNotify (text='', desp='') {
    const PUSH_PLUS_BY_TOKEN = process.env.PUSH_PLUS_BY_TOKEN;
    // 开启一对一推送
    if (PUSH_PLUS_BY_TOKEN) {
        const isEsisted = await isFileExisted(`${configPath}/scripts/pushToken.json`);
        if (!isEsisted) {
            console.log(`请先完善${configPath}/scripts 下的pushToken.json文件`)
            console.log(`格式如下：注意事项：请务必到 http://www.json.cn 网站检查 pushToken.json 文件格式是否正确
            {
                "tokens":[
                    {
                        "names":["用户名","昵称"],// 各个脚本用户名，能匹配到就发
                        "token":"d69485fa9f74437ca258f102"//pushPlus token
                    }
                ],
                "pushAllByTitle": ["京喜工厂商品列表详情","京东白嫖榜"],//以下标题全员推送
                "disableByTitle": ["京东到家果园信息"],//以下标题全员不推送
                "psuhOwner":["CK顺序调试工具"],// 以下标题只推送车主
                "disableStartTime":"01:32:00",//以下时间段只推送车主
                "disableEndTime":"08:00:00"//以下时间段只推送车主
            }`)
            return;
        }
        ...
        let tokenInfo = {};
        try {
          tokenInfo = await readFile(`${configPath}/scripts/pushToken.json`);
          tokenInfo = JSON.parse(tokenInfo);
        } catch {
          console.log(`格式不正确，取消推送。请务必到 http://www.json.cn 网站检查 pushToken.json 文件格式是否正确`)
          return
        } 
        
        // 设置了禁止推送时间段(因为部分信息会在0-5点推送，会打扰到老人或父母, 故只推送车主)
        if (tokenInfo.disableStartTime && tokenInfo.disableEndTime) {
            const date = new Date();
            ...
            if (dateTimes > start && dateTimes<end) {
                console.log('到了禁止推送时间了，只推车主');
                pushPlusNotify1(text, desp, tokenInfo.tokens[0].token)
                return; 
            }
        }

        // 以下标题只推送车主
        if (tokenInfo.psuhOwner.includes(text)) {
            pushPlusNotify1(text, desp, tokenInfo.tokens[0].token)
            return;
        }

        // 以下标题全员推送
        if (tokenInfo.pushAllByTitle.includes(text)) {
            tokenInfo.tokens.forEach(i => {
                pushPlusNotify1(text, desp, i.token)
            })
            return;
        }

        // 以下标题全员不推送
        if (tokenInfo.disableByTitle.includes(text)) return;
        ...
        console.log('\n');
        console.log('=================== 已处理信息 ===================');
        const despList = desp.split('\n\n');
        console.log(despList)
        console.log('\n');

        let token = '';
        let content = '';
        for (let i=1;i<despList.length;i++) { // i=1 车主最后完整推送
          content = despList[i];
          let name = content.slice(0, 25);
          console.log('前25字符：', name)
          for(let k=0;k<tokenInfo.length;k++){
            for(let j=0;j<tokenInfo[k].names.length;j++) {
              if (name.includes(tokenInfo[k].names[j])) {
                token = tokenInfo[k].token;
                break;
              }
            }
            if (token) break;
        ...
        console.log('即将推送：', `${token}`);
        if(token && content) {
          pushPlusNotify1(text, content, token);
        }
        // 完整信息，默认推送给车头
        pushPlusNotify1(text, desp, tokenInfo.tokens[0].token)
        
    } else {
        console.log('若需要开启一对一精准推送，请在配置文件中添加：export PUSH_PLUS_BY_TOKEN="true"')
        // 原逻辑
        pushPlusNotify1(text, desp, PUSH_PLUS_TOKEN)
    }
}
